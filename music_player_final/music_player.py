# -*- coding: utf-8 -*-
"""
Created on Thu Dec 25 18:05:45 2025
...
class Database:
    ...
    def get_most_played(self):
        self.cursor.execute("""
            SELECT path FROM songs ORDER BY play_count DESC;
        """)
        return [row[0] for row in self.cursor.fetchall()]

    def get_recently_added(self):
        self.cursor.execute("""
            SELECT path FROM songs ORDER BY id DESC;
        """)
        return [row[0] for row in self.cursor.fetchall()]
    ...
    def save_features(self, path, features_dict):
        """Store ML features (MFCC, chroma, embeddings, etc.)"""
        self.cursor.execute("""
            UPDATE songs
            SET features = %s
            WHERE path = %s;
...
class MusicPlayer:
    ...
    def sorted_list(self, songs):
        ...
        if self.sort_mode == "Recently Added":
            ...
            return sorted(songs, key=lambda p: index_map.get(p, 1_000_000))
        return songs

    def filtered_list(self):
        songs = self.base_list()
        songs = self.sorted_list(songs)
    ...
    def set_volume(self, v):
        for s in AudioUtilities.GetAllSessions():
            if s.Process and s.Process.name() == "python.exe":
                s._ctl.QueryInterface(ISimpleAudioVolume).SetMasterVolume(float(v), None)
    ...
    def apply_theme(self, theme):
        ...
        for b in self.buttons:
            cfg = {}
            if b_fg: cfg["fg_color"] = b_fg
            if b_hv: cfg["hover_color"] = b_hv
            if b_tx: cfg["text_color"] = b_tx
            if cfg: b.configure(**cfg)
        ...
        for s in self.sliders:
            cfg = {}
            if s_fg: cfg["fg_color"] = s_fg
            if s_pr: cfg["progress_color"] = s_pr
            if s_bt: cfg["button_color"] = s_bt
            if s_hv: cfg["button_hover_color"] = s_hv
        ...
        for t in self.textboxes:
            cfg = {}
            if t_fg: cfg["fg_color"] = t_fg
            if t_tx: cfg["text_color"] = t_tx
            if cfg: t.configure(**cfg)
        ...
        for c in self.combos:
            cfg = {}
            if c_fg: cfg["fg_color"] = c_fg
            if c_bd: cfg["border_color"] = c_bd
            if c_bt: cfg["button_color"] = c_bt
            if c_hv: cfg["button_hover_color"] = c_hv
            if c_tx: cfg["text_color"] = c_tx
            if cfg: c.configure(**cfg)
    ...
    def update_progress(self):
        if pygame.mixer.music.get_busy() and not self.dragging:
            pos = pygame.mixer.music.get_pos() / 1000
            if self.song_duration > 0:
                self.progress.set((pos / self.song_duration) * 100)
            self.app.after(100, self.update_progress)

    def seek(self, v):
        if self.song_duration > 0:
            pygame.mixer.music.play(start=(float(v) / 100) * self.song_duration)
        self.dragging = False
    ...
    def build_ui(self):
        ...
        self.favorite_btn = CTkButton(controls, text="♡", width=60, command=self.toggle_favorite, font=("Arial", 16))
        self.favorite_btn.pack(side="left", padx=8, pady=10)
        self.buttons.append(self.favorite_btn)

        del_btn = CTkButton(controls, text="🗑 Delete", width=80, command=self.delete_song)
        ...
        self.volume_slider = CTkSlider(left, from_=0, to=1, width=320, command=lambda v: self.set_volume(v))
        ...
        self.progress.grid(row=1, column=0, columnspan=2, sticky="ew", padx=20, pady=(30, 10))
        ...
        search_entry = CTkEntry(right, placeholder_text="Search...", textvariable=self.search_var, width=220)
        ...
        load_btn = CTkButton(right, text="🎵 Load Songs", command=self.load_songs, width=200, corner_radius=12)
        load_btn.pack(pady=5)
        self.buttons.append(load_btn)

        folder_btn = CTkButton(right, text="📁 Load Folder", command=self.load_folder, width=200, corner_radius=12)
        ...
        view_box = CTkComboBox(bottom, values=["All", "Favorites"], width=140, command=self.change_view)
        view_box.set("All")
        ...
        sort_box = CTkComboBox(bottom, values=["A-Z", "Z-A", "Most Played", "Recently Added"], width=180, command=self.change_sort)
        sort_box.set("A-Z")
        ...
        theme_box = CTkComboBox(bottom, values=theme_files, width=200, command=self.change_theme)
        ...
        mode_box = CTkComboBox(bottom, values=["Light", "Dark"], width=120, command=self.change_mode)
        mode_box.set("Light")
