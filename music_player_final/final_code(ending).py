...
#PART2= ML — EMBEDDINGS, CLUSTERING, RECOMMENDATIONS

def extract_embedding(path):
    """Extract a 40-d MFCC embedding (20 mean + 20 std)."""
    try:
...
def extract_all_embeddings(db: Database):
    """Extract embeddings for songs missing them."""
    for path in db.get_songs():
        features = db.get_features(path)
        if features and "embedding" in features:
            continue
        emb = extract_embedding(path)
        if emb:
            db.save_features(path, {"embedding": emb})
            print("Saved embedding:", path)


def run_clustering(db: Database, n_clusters=5):
    """Cluster songs using KMeans based on embeddings."""
    db.cursor.execute("""
    ...
    for path, emb, cluster_id in zip(paths, X, labels):
        db.save_features(path, {"embedding": emb.tolist(), "cluster": int(cluster_id)})

    print("Clustering complete.")


def recommend_songs(db: Database, top_n=10):
    """Return top-N recommended songs based on favorites and cluster + similarity."""
    db.cursor.execute("""
        SELECT path, features
        FROM songs
        WHERE features IS NOT NULL
    ...
    for path, features in rows:
        if path in favs:
            fav_embs.append(features["embedding"])
            fav_clusters.add(features["cluster"])

    if not fav_embs:
        return []

    fav_embs = np.array(fav_embs)

    candidates = []
    cand_embs = []

    for path, features in rows:
        if path not in favs and features["cluster"] in fav_clusters:
            candidates.append(path)
            cand_embs.append(features["embedding"])

    if not cand_embs:
        return []

    cand_embs = np.array(cand_embs)
    sims = cosine_similarity(cand_embs, fav_embs)
    avg_sim = sims.mean(axis=1)

    idx = np.argsort(avg_sim)[::-1][:top_n]
    return [(candidates[i], float(avg_sim[i])) for i in idx]
...
class MusicPlayer:
    def __init__(self):
        ...
        self.app = CTk()
        self.app.title("Music Player (Compact)")
        ...
        # Grid:
        # row 0: left + right panels
        # row 1: progress bar + time
        # row 2: bottom bar
        self.app.grid_rowconfigure(0, weight=4)
        self.app.grid_rowconfigure(1, weight=1)
        self.app.grid_rowconfigure(2, weight=0)
        self.app.grid_columnconfigure(0, weight=4)
        ...
        self.is_playing = False  # for toggle behavior
...
# VOLUME (PYCAW)
    ...
    def extract_album_art(self, path):
        try:
            ...
            for tag in audio.tags.values():
                if hasattr(tag, "data"):
                    # Resize smaller: 220x220
                    return Image.open(BytesIO(tag.data)).resize((220, 220))
        except:
            return None
        return None
    ...
    def update_image(self, path=None):
        if path and os.path.exists(path):
            ...
            if img is None:
                img = Image.new("RGB", (220, 220), "gray")
        else:
            img = Image.new("RGB", (220, 220), "gray")
    ...
    def update_favorite_button(self):
        if not self.song_list or self.current_song is None:
            self.favorite_btn.configure(text="❤️ Fav")
            return
        ...
        if path in favs:
            ...
            self.favorite_btn.configure(text="♡ Fav")

    def favorite_pop(self):
        if self.favorite_animating or not self.favorite_btn:
            return
        self.favorite_animating = True
        self.favorite_btn.configure(font=("Arial", 14, "bold"))

        def shrink():
            self.favorite_btn.configure(font=("Arial", 12))
            self.favorite_animating = False

        self.app.after(120, shrink)
    ...
    def internal_play(self, i):
        """Play index i (no toggle logic)."""
    ...
    def play_from_index(self, i):
        self.internal_play(i)

    def play_song_from_path(self, path):
        if path in self.song_list:
            idx = self.song_list.index(path)
            self.internal_play(idx)
    ...
    def toggle_play_pause(self):
        ...
        if not pygame.mixer.music.get_busy() and not self.is_playing:
            if self.current_song is None:
                self.internal_play(0)
            else:
                self.internal_play(self.current_song)
        else:
            if self.is_playing:
    ...
    def next(self):
        if self.song_list:
            idx = 0 if self.current_song is None else (self.current_song + 1) % len(self.song_list)
            self.internal_play(idx)
    ...
    def on_playlist_click(self, event):
        ...
        if 0 <= line < len(self.display_list):
            ...
            if path in self.song_list:
                self.internal_play(self.song_list.index(path))
...
#PART4: ML UI — POPUP RECOMMENDATIONS

    def show_ml_recommendations(self):
        recs = recommend_songs(self.db, top_n=10)

        popup = CTkToplevel(self.app)
        popup.title("ML Recommendations")
        popup.geometry("500x500")

        title = CTkLabel(
            popup,
            text="ML Recommendations (Based on Favorites)",
            font=("Arial", 20, "bold"),
        )
        title.pack(pady=10)

        frame = CTkScrollableFrame(popup, width=460, height=380, corner_radius=10)
        frame.pack(pady=10, padx=10)

        if not recs:
            CTkLabel(
                frame,
                text="No ML recommendations available.\n\n"
                     "Tips:\n"
                     "- Mark some songs as favorites ❤️\n"
                     "- Run feature extraction + clustering.",
                font=("Arial", 14),
                justify="left",
            ).pack(pady=20, padx=10, anchor="w")
        else:
            for path, score in recs:
                row = CTkFrame(frame, corner_radius=8)
                row.pack(fill="x", pady=4, padx=4)

                name = os.path.basename(path)
                txt = f"{name}  ({score:.3f})"
                lbl = CTkLabel(row, text=txt, font=("Arial", 14), anchor="w")
                lbl.pack(side="left", padx=10, pady=6, fill="x", expand=True)

                def make_play_callback(p=path, win=popup):
                    def _inner():
                        self.play_song_from_path(p)
                        win.destroy()
                    return _inner

                btn = CTkButton(row, text="▶ Play", width=80, command=make_play_callback())
                btn.pack(side="right", padx=10, pady=4)

        close_btn = CTkButton(popup, text="Close", width=120, command=popup.destroy)
...
#PART5:BUILD UI
    ...
    def build_ui(self):
        ...
        title = CTkLabel(left, text="Now Playing", font=("Arial", 22, "bold"))
        title.grid(row=0, column=0, pady=(6, 2))
        self.labels.append(title)

        self.cover = CTkLabel(left, text="")
        self.cover.grid(row=1, column=0, pady=10)
        self.labels.append(self.cover)

        controls = CTkFrame(left, corner_radius=15)
        controls.grid(row=2, column=0, pady=(2, 2))
        ...
        prev_btn.pack(side="left", padx=4, pady=4)
        ...
        self.play_pause_btn.pack(side="left", padx=4, pady=4)
        ...
        next_btn.pack(side="left", padx=4, pady=4)
        ...
        self.favorite_btn.pack(side="left", padx=4, pady=4)
        ...
        vol_label = CTkLabel(left, text="Volume", font=("Arial", 13))
        vol_label.grid(row=3, column=0, pady=(3, 0))
        ...
        self.volume_slider.configure(progress_color="#1f6aa5", button_color="#1f6aa5")
        self.volume_slider.grid(row=4, column=0, pady=(3, 6))
        ...
        # ---------- PROGRESS ----------
        pb_label = CTkLabel(self.app, text="Playback", font=("Arial", 14))
        pb_label.grid(row=1, column=0, sticky="w", padx=20)
        self.labels.append(pb_label)

        self.progress = CTkSlider(self.app, from_=0, to=100, command=self.seek)
        self.progress.grid(
            row=1, column=0, columnspan=2, sticky="ew", padx=18, pady=(24, 4)
        )
        self.progress.bind("<Button-1>", self.start_seek)
        self.progress.configure(progress_color="#1f6aa5")
        self.sliders.append(self.progress)

        self.time_label = CTkLabel(self.app, text="00:00 / 00:00", font=("Arial", 12))
        self.time_label.grid(row=1, column=0, columnspan=2, sticky="e", padx=22, pady=(0, 6))
        ...
        right.grid(row=0, column=1, sticky="nsew", padx=16, pady=12)
        right.grid_rowconfigure(2, weight=1)
        right.grid_columnconfigure(0, weight=1)
        self.frames.append(right)

        search_entry = CTkEntry(
            right, placeholder_text="Search...", textvariable=self.search_var, width=260
        )
        search_entry.grid(row=0, column=0, pady=(8, 4), padx=10, sticky="ew")
        self.search_var.trace_add("write", self.on_search_change)

        pl_label = CTkLabel(right, text="Playlist", font=("Arial", 20, "bold"))
        pl_label.grid(row=1, column=0, pady=(4, 4))
        self.labels.append(pl_label)

        self.playlist = CTkTextbox(right, width=260, height=360, corner_radius=10)
        self.playlist.grid(row=2, column=0, pady=8, padx=10, sticky="nsew")
        self.playlist.bind("<Button-1>", self.on_playlist_click)
        self.textboxes.append(self.playlist)

        btn_frame = CTkFrame(right, fg_color="transparent")
        btn_frame.grid(row=3, column=0, pady=(4, 8))
        self.frames.append(btn_frame)

        load_btn = CTkButton(
            btn_frame, text="🎵 Load Songs", command=self.load_songs, width=200, corner_radius=12
        )
        load_btn.pack(pady=2)
        self.buttons.append(load_btn)

        folder_btn = CTkButton(
            btn_frame, text="📁 Load Folder", command=self.load_folder, width=200, corner_radius=12
        )
        ...
        bottom.grid(row=2, column=0, columnspan=2, pady=6, padx=16, sticky="ew")
        self.frames.append(bottom)

        v_lbl = CTkLabel(bottom, text="View:", font=("Arial", 13))
        v_lbl.pack(side="left", padx=(10, 4))
        ...
        s_lbl = CTkLabel(bottom, text="Sort:", font=("Arial", 13))
        s_lbl.pack(side="left", padx=(16, 4))
        ...
        t_lbl = CTkLabel(bottom, text="Theme:", font=("Arial", 13))
        t_lbl.pack(side="left", padx=(16, 4))
        ...
        theme_box.pack(side="left", padx=4)
        self.combos.append(theme_box)

        m_lbl = CTkLabel(bottom, text="Mode:", font=("Arial", 13))
        m_lbl.pack(side="left", padx=(16, 4))
